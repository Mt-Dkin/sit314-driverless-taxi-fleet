 ...
 * assignment) routed from the Node-RED Event Router (via its own SQS
 * queue in AWS). Tracks active rides and produces a simple fare
 * calculation when a ride ends. Writes an immutable event history record
 * for each ride event (DynamoDB in production; in-memory array here for
 * local dev).
 */
...
function processRideEvent(payload) {
  ...
  if (ride_id && passenger_status === "occupied" && !activeRides.has(ride_id)) {
    ...
    if (passenger_status === "empty") {
      ...
      return { ok: true, ride_completed: true, fare };
    }
  }
  return { ok: true, active_rides: activeRides.size };
}

app.post("/ride-events", (req, res) => {
  res.status(200).json(processRideEvent(req.body));
});
...
startSqsConsumer({
  ...
  region: AWS_REGION,
  logPrefix: "[dispatch-billing]",
  handler: async (payload) => processRideEvent(payload),
});
